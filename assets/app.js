...
async function loadCredits(){
  const email = getEmail();
  creditRowExists = false;
  if(!supabaseClient || !email) return { exists: false, balance: getCredits() };
  const result = await supabaseClient.from('credits').select('balance').eq('email', email).limit(1);
  if(!result.error && typeof result.data?.[0]?.balance === 'number'){
    creditRowExists = true;
    setCredits(result.data[0].balance);
    return { exists: true, balance: result.data[0].balance };
  }
  return { exists: false, balance: getCredits() };
}

async function saveCredits(){
  const email = getEmail();
  if(!supabaseClient || !email) return false;
  const result = await supabaseClient.from('credits').upsert({
    email,
    balance: getCredits(),
    updated_at: new Date().toISOString()
  }, { onConflict: 'email' });
  if(result.error){
    console.error('Credit save error:', result.error);
    return false;
  }
  creditRowExists = true;
  return true;
...
async function grantAutomaticTrial(){
  ...
  let alreadyRecorded = !supabaseClient && trialEmails().includes(email);
  ...
  if(alreadyRecorded){
    markTrial(email);
    if(!creditRowExists && supabaseClient){
      const repairedBalance = await rebuildTrialBalanceFromUsage(email, trialCredits);
      setCredits(repairedBalance);
      await saveCredits();
      return repairedBalance > 0;
    }
    return false;
  }
  ...
  if(supabaseClient){
    const inserted = await supabaseClient
      .from('orders')
      .insert({ email, plan: 'trial', amount: 0, credits: trialCredits, status: 'trial' });

    if(inserted.error) console.error('Automatic trial record error:', inserted.error);
  }

  return true;
}

async function rebuildTrialBalanceFromUsage(email, trialCredits){
  const detailed = await supabaseClient
    .from('usage_logs')
    .select('cost,balance_after,created_at')
    .eq('email', email)
    .order('created_at', { ascending: false });

  if(!detailed.error){
    const rows = detailed.data || [];
    const latestBalance = rows.find(row => typeof row.balance_after === 'number')?.balance_after;
    if(typeof latestBalance === 'number') return Math.max(0, latestBalance);
    const spent = rows.reduce((total, row) => total + (Number(row.cost) || 0), 0);
    return Math.max(0, trialCredits - spent);
  }
  ...
  if(!basic.error){
    const spent = (basic.data || []).reduce((total, row) => total + (Number(row.cost) || 0), 0);
    return Math.max(0, trialCredits - spent);
  }
...
async function generateAI(){
  ...
  const prompt = `Tool requested: ${tool}
Product/topic: ${product}
Seller details:
${details}

Write the final customer-ready ecommerce output only.
Do not explain what SaleSmart AI is.
Do not introduce the tool.
Do not say "hypothetical", "this output demonstrates", or "the AI generated".
...
Make the answer complete and useful. For listing, research, review, keyword, and multilingual tools, write 900 to 1,300 words unless the seller details are very short.

If this is a listing generator, start exactly with:
Optimized Title
...
For the listing title, use the real product/topic words and features provided. Do not invent a fake brand name unless the seller provided one.
For every other tool, start directly with the useful result and include clear sections, examples, and next actions.`;
