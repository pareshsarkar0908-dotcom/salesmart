...
function trackSalesmartEvent(name, params={}){
...
async function signup(){
  ...
  try {
    ...
    trackSalesmartEvent('account_created', { method: 'email' });
    setTimeout(() => {
...
async function grantAutomaticTrial(){
  ...
  let alreadyRecorded = trialEmails().includes(email);
  ...
  if(creditRowExists && getCredits() > 0){
    return false;
  }

  const startingCredits = getCredits();
  markTrial(email);
  addCredits(trialCredits);
  const saved = await saveCredits();
...
For every other tool, start directly with the useful result and include clear sections, examples, and next actions.`;
  ...
  await saveCredits();
  trackSalesmartEvent('ai_generation', { tool, value: 1 });
  if(supabaseClient){
